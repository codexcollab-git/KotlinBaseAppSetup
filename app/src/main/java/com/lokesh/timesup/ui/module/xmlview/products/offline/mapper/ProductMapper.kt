package com.lokesh.timesup.ui.module.xmlview.products.offline.mapper
...
class ProductMapper {
    ...
    fun mapProductToProductsEntity(list: List<Product>?): List<ProductEntity> {
        ...
        return list.map {
            ...
            ProductEntity(id = it.id ?: 0, title = it.title, description = it.description, price = it.price, discountPercentage = it.discountPercentage,
    ...
    fun mapProductsEntityEntityToProduct(list: List<ProductEntity>?): List<Product> {
        ...
        return list.map {
            ...
            Product(id = it.id, title = it.title, description = it.description, price = it.price, discountPercentage = it.discountPercentage,
                rating = it.rating, stock = it.stock, brand = it.brand, category = it.category, thumbnail = it.thumbnail, images = imageList)
        }
    }
}
package com.lokesh.timesup.ui.module.xmlview.categories.view.fragment
...
import com.lokesh.timesup.databinding.FragmentCategoriesListBinding
import com.lokesh.timesup.model.Categories
import com.lokesh.timesup.ui.module.xmlview.categories.adapter.CategoriesAdapter
import com.lokesh.timesup.ui.module.xmlview.categories.viewmodel.CategoriesViewModel
import com.lokesh.timesup.util.BACK_CATEGORY_DATA
import com.lokesh.timesup.util.BACK_PRODUCT_LIST_TO_CATEGORY_LIST
import com.lokesh.timesup.util.CATEGORY_NAME
...
import com.lokesh.timesup.util.showSnackBar
...
class CategoriesListFragment : BaseFragment<FragmentCategoriesListBinding>() {
    ...
    private val categoriesObserver = Observer<ArrayList<Categories>?> {
        if (!it.isNullOrEmpty() && NetworkManager.internetAvailable) {
            val list = it.map { li -> li.name!! }
            categoryAdapter?.fillCategories(list)
            showSnackBar("Categories have been loaded from online", false, requireActivity())
        }
    }

    private val categoriesOfflineObserver = Observer<List<String>?> {
        if (!it.isNullOrEmpty() && !NetworkManager.internetAvailable) {
    ...
    private fun navigateProductList(category: String) {
        val args = Bundle().also { it.putString(CATEGORY_NAME, category) }

package com.lokesh.timesup.util.prefs

import android.content.SharedPreferences
import com.google.gson.Gson
import com.lokesh.timesup.util.encryption.Encryption_Local
...
class PrefUtils(private val prefs: SharedPreferences) {
    ...
    private fun getEncryption(): Encryption_Local {
        ...
        return Encryption_Local.getDefault(PrefsKeys.ENCRYPTION_KEY.toString(), PrefsKeys.ENCRYPTION_SALT.toString(), byteSize)
    }

    var authToken: String?
        get() = String.format("%s", getEncryption().decryptOrNull(prefs.getString(PrefsKeys.AUTH_TOKEN.toString(), null)))
